...
fun FutureTasksScreen(
    ...
    val viewModel: FutureTasksViewModel = viewModel(
        factory = FutureTasksViewModelFactory(context)
    )

    DisposableEffect(Unit) {
        viewModel.fetchAllFutureTasks()
        onDispose { }
    }

    val tasks = convertToTaskItemData(viewModel.tasks?.getOrNull() ?: emptyList(),"Upcoming")
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
